...
<head>
<meta charset="utf-8">
<title>Police Emergency Services System</title>
	<?php
	...
	$mysqli = mysqli_connect(DB_SERVER, DB_USER, DB_PASSWORD, DB_DATABASE);
		...
		$sql = "SELECT incidentId FROM disptach WHERE timeCompleted IS NULL AND patrolcarId = ?";
		...
		if (!($stmt = $mysqli->prepare($sql))) {
			...
			AND NOT EXISTS (SELECT * FROM dispatch WHERE timeCommpleted IS NULL AND incidentId = '$incidentId')";
...
<body>
	...
	<!--diaplay a form for operator to update status of patrol car-->
